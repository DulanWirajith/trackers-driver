...
import com.example.driver03.model.BackgroundTask;
...
public class login extends AppCompatActivity {
    ...
    private void loginUser() {
        ...
        loginRequest.setDriver_mail(email.getText().toString());
            ...
            public void onResponse(Call<LoginResponse> call, Response<LoginResponse> response) {
                ...
                if (response.isSuccessful()) {
                    if (response.body().isUserRight()) {
                        ...
                        String driverFirstName = response.body().getDriverFirstName();
                        String driverLastName = response.body().getDriverLastName();
                        String driverMail = response.body().getDriverMail();
                        String driverContact = response.body().getDriverContact();
                        boolean driverIsVertify = response.body().isDriverIsVertify();
                        String driverLicence = response.body().getDriverlicenceNumber();
                        int driverOverAllScore = response.body().getDriverOverAllScore();


                        BackgroundTask backgroundTask = BackgroundTask.setBackgroundTask(driverID, driverFirstName, driverLastName, driverMail, driverContact, driverIsVertify, driverLicence, driverOverAllScore);
